...
- **Current blocker:** no implementation blocker is known; exact-head CI, the authoritative frontend preview and checklist review must pass before any separate Finalize decision.
- **Exact next command:** `Review CW-017 exact-head CI and pull requests`; stop on a regression or authoritative-preview failure and do not merge or deploy.
...
| CW-017 | Reservation Awareness Phase 2A | Add the booking-target contract and deterministic booking-window engine | IN_PROGRESS | User + Codex | Additive/version-tolerant targets normalize safely; opening/deadline calculations expose rule provenance, as-of/verification state and per-date overrides; existing reservations, shared sync/history, itinerary control, family-key recovery and production data remain unchanged | None | IN_REVIEW | [issue #90](https://github.com/MileHighHoosier/castlewatch-2027/issues/90), [frontend PR #57](https://github.com/MileHighHoosier/castlewatch-frontend/pull/57), [documentation PR #91](https://github.com/MileHighHoosier/castlewatch-2027/pull/91), [checkpoint](docs/reservation-awareness-phase-2a.md) | 2026-09-08 | Review both PRs, exact-head CI and the authoritative frontend preview; do not merge, deploy, finalize 2A or begin Phase 2B. |
