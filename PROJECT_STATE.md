...
**In progress; Section 8A is finalized and the Section 8B implementation merged August 31, 2026. The separate Section 8B test checklist is next.**
...
Sections 1–7 and Section 8A are complete. Section 8B implementation is merged; its separate test checklist and Finalize checkpoint remain pending. Keep `CASTLEWATCH_FAMILY_KEY` configured and enabled; no later retirement option is authorized without a separate explicit user approval.

## Exact next priorities

1. **Run `Start Section 8B test checklist`.**
2. Verify the merged route model across frontend/backend gates and read-only production behavior before any Section 8B Finalize request.
