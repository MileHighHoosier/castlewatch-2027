...
_Last rebaseline: September 8, 2026_
...
A September 7, 2026 repo-first audit found five bounded issues that had to be corrected before Phase 2A could introduce booking-target fields:
...
Initial frontend head `eb2d177321b61b899da2116c7e95aff60acde23a` passed all contracts and its production build in exact-head CI run `34801378007`, but the rendered smoke read stored rows before proving that the real cross-tab lock queue had drained and reported `add preserves the other tab's addition` as `0 !== 1`. Deterministic writer coverage remained green. The bounded correction at `eaaf71c0cbdb8cfecb4634c45ed3c41b32c909d1` changes no application code: the smoke completion barrier now requires no held or pending booking-target lock plus the rendered idle state, and a deterministic regression protects that barrier. Local evidence is 31 focused and 181 full contracts plus the production build; local browser launch remains unavailable because Chrome is not installed. Corrected exact-head CI/Vercel evidence and independent review remain required. Phase 2D, production/shared-plan mutation, dependency/runtime work and obsolete Vercel-project work remain out of scope.

## Current development phase

**Reservation Awareness Phase 2C (CW-019) — published in frontend PR #59 and documentation PR #97; exact-head evidence and independent review pending**
...
1. Verify the exact heads and all associated frontend/backend CI plus the authoritative `castlewatch-frontend` preview for PRs #59 and #97.
2. Perform an independent Phase 2C review before any Finalize command; do not merge or deploy during review.
