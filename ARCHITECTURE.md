...
The backend continues to store and version the shared client plan without interpreting Disney booking-window policy. Additive client normalization must accept payloads with no booking-target data and Phase 2A targets with no Phase 2C history fields. Browser-local planner mutations acquire the origin-scoped exclusive Web Lock `castlewatch.booking-targets.v1.write` before reading, validating, applying their bounded operation and saving. This serializes overlapping participating tabs, preserves other valid targets/unknown fields and blocks malformed or future-format storage even if it arrived while a write waited. Explicit shared download and history restore use the same lock around their existing replacement transaction; authorization, payload semantics and Phase 2A synchronous calculation/serialization/raw-replacement helpers are unchanged. Missing Web Locks support fails closed without writing. Planner inputs are captured when events occur; pending UI drafts are not persisted arrays and are reconciled after success/failure and storage/focus notifications. Web Locks are cooperative: old pre-correction tabs must be reloaded/closed before a later rollout, and future app writers must use the same lock; arbitrary external or old nonparticipating writers are not made atomic by this protocol. No database-schema change is assumed.

Phase 2C stores optional dated attempt/result records and one explicit user-selected fallback choice on each target. These records describe family actions and observations; they are not inventory or official availability feeds. A target may enter booked state only through an explicit selection of an existing reservation that is revalidated at write time. The planner stores the reservation identifier but never creates, confirms, edits or deletes that reservation. A missing/deleted link or a link inconsistent with target state is rendered as a warning without cleanup. The reservation remains authoritative for booked date/time/location, transportation, conflicts and Trip Week decision effects; lifecycle target changes have none of those effects by themselves.
