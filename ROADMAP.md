...
Status: **In progress - 5A–5B complete; 5C next**
...
- **5C - normal shared-plan dual authorization and role enforcement: Next - not started.** Add device-token authorization for normal read/write/history/version/restore/operations paths with the exact Owner/Editor/Viewer server and UI matrix while preserving version/conflict/history behavior.
