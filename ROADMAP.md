...
Status: **In progress - 5A–5C complete; 5D next**
...
- **5D - revocation, recovery and legacy-gate hardening: Next - not started.** Enforce revoked-token denial everywhere without silent fallback, make the enabled legacy flag authoritative, add owner/last-owner and pepper-continuity protections, and verify recovery/rollback without adding retirement behavior.
